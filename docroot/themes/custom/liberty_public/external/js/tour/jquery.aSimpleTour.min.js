!(function ($) {
  var settings = {
      ...
      controlsCss: { background: 'rgba(8, 68, 142, 0.80)', color: '#fff' },
      tooltipCss: { background: 'rgba(0, 0, 0, 0.70)', color: '#fff' },
    },
    options,
    step,
    steps,
    ...
    methods = {
      init: function (a, b) {
        ;(options = $.extend(settings, a)),
          (b = 'undefined' == typeof b ? null : b - 1),
          0 == started &&
            ((started = !0),
            ...
            (controls =
              '<div id="tourControls">                  <div id="tourText">' +
              options.welcomeMessage +
              '</div>                  <div id="tourButtons">                    <button id="tourPrev" style="display:none" class="' +
              options.buttons.prev.class +
              '">' +
              options.buttons.prev.text +
              '</button>                    <button id="tourNext" class="' +
              options.buttons.start.class +
              '">' +
              options.buttons.start.text +
              '</button>                    <button id="tourEnd" style="display:none" class="' +
              options.buttons.end.class +
              '">' +
              options.buttons.end.text +
              '</button>                  </div>                </div>'),
            ($controlsCss = {
              display: 'block',
              position: 'fixed',
          ...
          null != b &&
            ((step = b),
            (stepData = options.data[b]),
            methods.setTooltip(step, stepData)),
          methods.bindElements()
      },
      next: function () {
        step > -1 &&
        options.data[step].hasOwnProperty('location') &&
        'undefined' != typeof options.data[step].location
      ...
      setTooltip: function (a, b, c) {
        options.useOverlay &&
          ($overlay ||
            ($overlay = $(
              '<div id="touroverlay" style="position: fixed; top: 0; left: 0; width: 100%; height: 100%; z-index: ' +
          ...
          ($previousElement = $(
            'next' == c
              ? options.data[a - 1].element
              : options.data[a + 1].element,
          )),
          'undefined' != typeof $previousElement.data('tour-data') &&
          ...
          null != b.tooltip &&
            ...
            b.text && $('#tourText').html(b.text),
            (tooltipPos = 'undefined' == typeof b.position ? 'BL' : b.position),
      ...
      getTooltipPosition: function (a, b) {
        switch (
          ...
          (th =
            $tooltip.height() +
            parseInt($tooltip.css('padding-top')) +
      ...
      getControlPosition: function (a) {
        switch (a) {
          case 'TR':
            a = { top: '10px', right: '10px' }
            break
          case 'TL':
            a = { top: '10px', left: '10px' }
            break
          case 'BL':
            a = { bottom: '10px', left: '10px' }
            break
          case 'BR':
      ...
      showHideButtons: function () {
        step < steps && $('#tourNext').show().html(options.buttons.next.text),
          step <= 0 &&
            ($('#tourPrev').hide(),
            $('#tourEnd').hide(),
            $('#tourNext')
              .html(options.buttons.start.text)
              .attr('class', options.buttons.start.class)),
          step <= steps &&
            step > 0 &&
            ($('#tourPrev').show(),
            $('#tourEnd').show(),
            $('#tourNext')
              .show()
              .html(options.buttons.next.text)
              .attr('class', options.buttons.next.class))
      },
      findParentBg: function (a) {
        for (; 'transparent' == a.css('background-color'); ) a = a.parent()
        return a.css('background-color')
      },
      destroy: function () {
        $('#tourControls').remove(),
          $('#tourtip').remove(),
          ...
          (started = !1),
          (options.keyboard = !1),
          options.useOverlay && $overlay.remove()
      },
      bindElements: function () {
        $('body').on('click', '#tourNext', function () {
          ...
          $('body').on('keydown', function (a) {
            37 == a.keyCode && options.keyboard ? methods.prev() : '',
              39 == a.keyCode && options.keyboard ? methods.next() : '',
              27 == a.keyCode && options.keyboard ? methods.destroy() : ''
          })
      },
    }
  ;($.fn.aSimpleTour = function (a) {
    return (
      (t = this),
      methods[a]
        ? methods[a].apply(this, Array.prototype.slice.call(arguments, 1))
        : 'object' != typeof a && a
        ? void $.error('Method ' + a + ' does not exist on jQuery.aSimpleTour')
        : methods.init.apply(this, arguments)
    )
  }),
    $(window).load(function () {
      ;(tourStep = window.location.hash.substr(1).split('=')),
        'undefined' != typeof eval(tourStep[0]) &&
          'undefined' != typeof eval(tourStep[1]) &&
          $.aSimpleTour(eval(tourStep[0]), tourStep[1])
    })
})(jQuery),
  ($.aSimpleTour = function (a, b) {
    return $.fn.aSimpleTour(a, b), this
  })
