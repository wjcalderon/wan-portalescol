...
(function () {
  let StyleData = CKEDITOR.tools.createClass({
    ...
    $: function (options) {
      let entries = options.entries.split(";");
    ...
    proto: {
      ...
      addToCombo: function (combo) {
        for (let i = 0; i < this._.names.length; i++) {
          let name = this._.names[i];
      ...
      getMatchingValue: function (editor, path) {
        ...
        for (let i = 0, element, value; i < elements.length; i++) {
          element = elements[i];
  ...
  function addCombo(editor, definition) {
    ...
    editor.addCommand(definition.commandName, {
      ...
      refresh: function (editor, path) {
        if (
          !defaultContentStyle.checkApplicable(
            path,
            editor,
            editor.activeFilter
          )
    ...
    editor.ui.addRichCombo(definition.comboName, {
      ...
      init: function () {
        let defaultText = "(" + editor.lang.common.optionDefault + ")";
  ...
  function isEqualStyle(styleA, styleB) {
    if (
      !(styleA instanceof CKEDITOR.style) ||
      !(styleB instanceof CKEDITOR.style)
    ) {
      return false;
    }

    let hasEqualAttributes = compareAttributes(styleA, styleB),
    ...
    function compareAttributes(styleA, styleB) {
      let styleAAttributes = styleA.getDefinition().attributes,
    ...
    function compareStyles(styleA, styleB) {
      return (
        CKEDITOR.style.getStyleText(styleA.getDefinition()) ===
        CKEDITOR.style.getStyleText(styleB.getDefinition())
      );
    }
  }
  ...
  function cloneSubtreeIntoRange(range, elements, subtreeStart) {
    ...
    if (subtreeStart) {
      return cloneSubtreeIntoRange(
        range,
        elements,
        current.equals(subtreeStart) ? null : subtreeStart
      );
    }
