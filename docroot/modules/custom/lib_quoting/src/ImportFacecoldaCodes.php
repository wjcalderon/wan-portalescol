    ...
    if ($bool_brand && $bool_line && $bool_code) {
      ...
      if (!$nid_vehicle) {
        ...
        if (!empty($item['G'])) {
          ...
          $tid_type_vehicle = get_tid_by_name($name_type_vehicle, 'tipo_vehiculo');
        }
        ...
        $values = [
          ...
          'field_tipo_vehiculo' => $tid_type_vehicle ? [
            'target_id' => $tid_type_vehicle,
          ] : [],
          'field_marca_linea' => $tid_brand_line ? [
    ...
    switch ($vid) {
      ...
      case 'referencia_adicional':
        if (!empty($item['F']) && !empty($item['J'])) {
          ...
          if ($vid == 'marca_linea') {
            $tid_brand_line = get_tid_by_name($name_brand_line, $vid);
            if (!$tid_brand_line) {
              Term::create([
            ...
            if (!empty($item['K'])) {
              $name_ref2 = trim($item['K']);
              $tid_ref2 = get_tid_by_name($name_ref2, $vid);
              $tid_brand_line = get_tid_by_name($name_brand_line, 'marca_linea');
