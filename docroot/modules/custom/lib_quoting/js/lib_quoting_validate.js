...
(function ($, Drupal) {
  ...
  let validate_numbers = function (number) {
    return /\D/.test(number);
  };

  // Validate form
  let validate_form = function (form) {
      ...
      elements = form
        ...
        .each(function (index, el) {
          ...
          if (!elm.attr("readonly") && !elm.attr("disabled")) {
            if (
              elm.attr("required") &&
              (elm.val() === "" || elm.val() === undefined)
            ) {
              errors.push("El campo es requerido");
            }
            if (
              elm.hasClass("form-email") &&
              !validate_email('"' + elm.val() + '"')
            ) {
            ...
            // Show errors
            elm.removeClass("error").parent().find("label.error").remove();
            if (errors.length > 0) {
  ...
  Drupal.behaviors.lib_quoting_validate = {
    attach: function (context) {
      ...
      $("#edit-num-plate").on("change", function () {
        let alNumRegex = /^([a-zA-Z0-9]+)$/;
        if (!alNumRegex.test($(this).val())) {
          $(this)
            .addClass("error")
            .parent()
            .append(
              '<label id="field-error" class="error" for="field">Solo se aceptan letras y n\xFAmeros</label>'
            );
          ...
          $(this).removeClass("error").parent().find("#field-error").remove();
        }
      });
