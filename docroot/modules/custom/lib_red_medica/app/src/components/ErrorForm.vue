<template>
  <section :class="['error-form', 'modal', (isMobile ? 'mobile modal-white' : '')]">
    <div :class="['modal-content', show_confirmation ? 'small' : '']">
      ...
      <form method="post" autocomplete="off" @submit="submitForm" :class="this.errors ? 'errors' : false"
        ...
        <div class="form-item js-form-type-text form-type-textfield form-item-name" ref="name">
          ...
          <input type="text" v-model="name" id="name" required class="form-text" maxlength="30" @invalid="invalidateForm"
            @change="labelClass('name')" @focus="labelClass('name')" @blur="removeLabelClass(name, 'name')">
        </div>
        <div class="form-item js-form-type-text form-type-textfield form-item-lastname" ref="lastname">
          <label for="lastname">Apellido</label>
          <input type="text" v-model="lastname" id="lastname" required class="form-text" maxlength="30"
            @invalid="invalidateForm" @change="labelClass('lastname')" @focus="labelClass('lastname')"
        ...
        <div class="form-item js-form-type-email form-type-email form-item-email" ref="email">
          ...
          <input type="email" v-model="email" id="email" required class="form-email" @invalid="invalidateForm"
            @change="labelClass('email')" @focus="labelClass('email')" @blur="removeLabelClass(email, 'email')">
        </div>
        <div class="form-item js-form-type-select form-type-select form-item-error_type" ref="error_type">
          ...
          <select class="form-select" id="error_type" v-model="error_type" required @invalid="invalidateForm"
            @change="labelClass('error_type')" @focus="labelClass('error_type')"
        ...
        <div class="form-item js-form-type-textarea form-type-textarea form-item-observation" ref="observation">
          ...
          <div>
            <textarea class="form-textarea required" id="observation" v-model="observation" rows="5" cols="60"
              autocomplete="off" @change="labelClass('observation')" @focus="labelClass('observation')"
        ...
        <div class="form-item js-form-type-checkbox form-type-checkbox form-item-personal_data">
          ...
          <label for="personal_data">
            Autorizo el uso de mis datos personales por parte de Liberty Seguros S.A. <a :href="personal_data_url"
              target="_blank" rel="noreferrer" title="Política de tratamiento de datos de Liberty Seguros">de acuerdo a
              sus términos, condiciones y políticas de privacidad</a>
          </label>
        </div>
        <fieldset class="bottom-form">
          <div>
            <p>Te estaremos notificando cuando el error haya sido revisado y solucionado</p>
            <input type="submit" value="Enviar" class="button" :disabled="!authorize_personal_data_use && !errors">
          </div>
        </fieldset>
      </form>
