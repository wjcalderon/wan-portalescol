...
/**
 * Class liberty validacion document.
 */
class LibertyDocumentValidationForm extends FormBase {
  ...
  public function __construct() {
    ...
    if (isset($variable_get_value)) {
      if ($variable_get_value == $variable_vip_name) {
        ...
        $url = Url::fromRoute('entity.node.canonical', ['node' => $path_args[2]], $options);
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $form['identification_type'] = [
      ...
      '#empty_option' => '',
      '#options' => [
        1 => $this->t("Cédula de ciudadanía"),
        2 => $this->t("Cédula de extranjería"),
        3 => $this->t("Pasaporte"),
      ],
      '#required' => TRUE,
      '#label_classes' => [
  ...
  public function validateIdCallback(array &$form, FormStateInterface $form_state): AjaxResponse {
    ...
    $form_state->getValue('identification_number');
    if (!$form_state->getErrors()) {
      ...
      // Si existe.
        ...
        $options = ['absolute' => TRUE];
        $url = Url::fromRoute('entity.node.canonical', ['node' => $campaign_id], $options);
