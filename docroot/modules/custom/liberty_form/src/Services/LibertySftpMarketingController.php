...
/**
 * @file
 * Contains \Drupal\liberty_form\Services\LibertySftpMarketingController
 */
...
class LibertySftpMarketingController extends ControllerBase
{
    ...
    public function doSomething($cid)
    {
        ...
        if ($config) {
            ...
            //crea el archivo csv  con la fecha y hora en la genera el archivo y le aplica permisos
            ...
            $url = \Drupal\Core\Url::fromRoute('entity.node.canonical', ['node' => $cid], $options);
            $url = $url->toString();
            ...
            if (count($results_customer) == 0) {
                ...
                $count = 1;
                $submission_data2 = [];
                $submission_data2[0] = array("Pais", "NombreSponsor", "ClaveNegocio", "Ramo", 'Campaña', 'Tipodocumento', 'Numerodocumento', 'SubscriberKey', 'Nombres', 'Apellidos', 'EmailCliente', 'Telefono', 'Ciudad', 'FechaNacimiento', 'Genero', 'Fechadecarguecampaña', 'UrlSponsor', 'CampaignLanding');
                ...
                foreach ($results_customer as $key => $item_customer) {
                    ...
                    $id_number = $entity->get('field_country')->getValue()[0]['value'];
                    ...
                    $url_landing = $url;
                    $term_name_sponsor = \Drupal\taxonomy\Entity\Term::load($entity->get('field_sponsor_name')->getValue()[0]['target_id'])->getName();
                    $term_name_ramo = \Drupal\taxonomy\Entity\Term::load($entity->get('field_branch')->getValue()[0]['target_id'])->getName();
                    ...
                    $node = \Drupal\node\Entity\Node::load($item_customer);
