 ...
 */
class ImportFormNissan extends FormBase {

  /**
   * {@inheritdoc}
   */
  public function getFormId() {
    return 'liberty_claims_import_nissan';
  }
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    if ($file) {
      try {
        ...
        $vocabulary_machine_name = 'talleres_nissan';
        ...
        foreach ($rows as $row) {
          ...
          $field_cod_taller_nissan = isset($rowData[1]) ? $rowData[1] : '';
          $field_aixis_nissan = isset($rowData[2]) ? $rowData[2] : '';
          $field_direccion_nissan = isset($rowData[3]) ? $rowData[3] : '';
          $field_nit_nissan = isset($rowData[4]) ? $rowData[4] : '';
          $field_sucursal_nissan = isset($rowData[5]) ? $rowData[5] : '';
          $field_telefono_nissan = isset($rowData[6]) ? $rowData[6] : '';
          $field_ciudad_nissan = isset($rowData[7]) ? $rowData[7] : '';
          $field_cod_ciudad_nissan = isset($rowData[8]) ? $rowData[8] : '';

          // Buscar el término existente por su Id del taller en el vocabulario
          $terms = \Drupal::entityTypeManager()->getStorage('taxonomy_term')->loadByProperties([
            'field_cod_taller_nissan' => $field_cod_taller_nissan,
          ...
          if (empty($terms)) {
            $term = Term::create([
              ...
              'vid' => $vocabulary_machine_name,
              'field_cod_taller_nissan' => $field_cod_taller_nissan,
              'field_aixis_nissan' => $field_aixis_nissan,
              'field_direccion_nissan' => $field_direccion_nissan,
              'field_nit_nissan' => $field_nit_nissan,
              'field_sucursal_nissan' => $field_sucursal_nissan,
              'field_telefono_nissan' => $field_telefono_nissan,
              'field_ciudad_nissan' => $field_ciudad_nissan,
              'field_cod_ciudad_nissan' => $field_cod_ciudad_nissan,
            ]);
            $term->save();
