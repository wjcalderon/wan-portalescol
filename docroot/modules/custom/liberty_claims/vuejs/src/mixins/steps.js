...
export default {
  ...
  methods: {
    ...
    isFormOk: function () {
      return !this.validationRules || !validator.validateForm(this.casualtyData, this.validationRules);
    },
    prevStep: function () {
    ...
    hasError: function (field) {
      return validator.validateField(this.casualtyData[field], this.validationRules[field]);
    },
    fileAdded: function ( file ) {
      const pdfLimit = ( parseFloat(this.drupalSettings.documentSize) * 1000) * 1000
      const imageLimit = ( parseFloat(this.drupalSettings.imageSize) * 1000) * 1000
    ...
    fileRemoved: function (file, error, xhr) {
      ...
      },
      {
        ...
        let tempFiles = this.casualtyData.files.filter((obj, index, list) => {
