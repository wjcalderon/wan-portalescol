  ...
  <div class="pane second-step">
    ...
    <div class="row columns-row">
      <div v-bind:class="{ field: true, error: submited && hasError('name'), filled: casualtyData.name }">
        <float-label>
          <input type="text" name="name" v-model.lazy="casualtyData.name" placeholder="¿Cuál es tu nombre?" tabindex=1
            maxlength="70" v-bind:disabled="personalInfo.name && personalInfo.name.length > 0">
        </float-label>
      ...
      <div v-bind:class="{ field: true, error: submited && hasError('lastname'), filled: casualtyData.lastname }">
        <float-label>
          <input type="text" name="lastname" v-model="casualtyData.lastname" placeholder="¿Cuales son tus apellidos?"
            tabindex=2 maxlength="70" v-bind:disabled="personalInfo.lastname && personalInfo.lastname.length > 0">
        </float-label>
      ...
      <div v-bind:class="{ field: true, error: submited && hasError('documentId'), filled: casualtyData.documentId }">
        <float-label>
          <input v-if="personalInfo.documentId && personalInfo.documentId.length > 0" type="text" name="id"
            placeholder="Escribe tu número de documento" tabindex=4 :value="idMask" disabled maxlength="50">
          <input v-else type="text" name="id" v-model="casualtyData.documentId"
            placeholder="Escribe tu número de documento" tabindex=4 maxlength="70">
        </float-label>
        <div class="error-message" v-show="submited && hasError('documentId')">{{ hasError('documentId') }}</div>
      </div>

      <div
        v-bind:class="{ field: true, address: true, error: submited && hasError('address'), filled: casualtyData.address }">
        <float-label>
          <input v-if="personalInfo.address && personalInfo.address.length > 0" type="text" name="address" maxlength="50"
            placeholder="¿Cuál es tu dirección?" tabindex=5 :value="addressMask" disabled>
          <input v-else type="text" name="id" maxlength="50" v-model="casualtyData.address"
            placeholder="¿Cuál es tu dirección?" tabindex=5>
        </float-label>
      ...
      <div v-bind:class="{ field: true, error: submited && hasError('phone'), filled: casualtyData.phone }">
        <float-label>
          <input type="text" v-model="casualtyData.phone" name="phone" placeholder="¿Cuál es tu número de teléfono?"
            tabindex=6>
        </float-label>
      ...
      <div v-bind:class="{ field: true, error: submited && hasError('email'), filled: casualtyData.email }">
        <float-label>
          <input type="mail" name="mail" v-model="casualtyData.email" placeholder="¿Cuál es tu email?" tabindex=7>
        </float-label>
      ...
      <div class="col">
        <div v-bind:class="{ field: true, error: submited && hasError('brand'), filled: casualtyData.brand }">
          <float-label label="¿Cuál es la marca de tu vehículo?" :dispatch="false">
            <search-select :options="brands" v-model="casualtyData.brand" tabindex=8
              placeholder="¿Cuál es la marca de tu vehículo?" v-bind:isDisabled="personalInfo.brand">
            </search-select>
          </float-label>
